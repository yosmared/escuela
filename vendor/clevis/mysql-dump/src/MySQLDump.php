...
class MySQLDump
{
	...
	private $customDataSelect = array();
	...
	/**
	 * Sets custom selection query for given table.
	 *
	 * @param  string
	 * @param  string
	 * @return self
	...
	public function write($handle = NULL)
	{
		...
		if ($this->useLock) {
			$delTables = array_map(array($this, 'delimite'), $tables);
			$this->connection->query('LOCK TABLES ' . implode(' READ, ', $delTables) . ' READ');
		}
	...
	public function dumpTable($handle, $table)
	{
		...
		if (!$view && ($mode & self::DATA)) {
			...
			$size = 0;
			$query = (isset($this->customDataSelect[$table]) ? $this->customDataSelect[$table] : "SELECT * FROM  $delTable");
